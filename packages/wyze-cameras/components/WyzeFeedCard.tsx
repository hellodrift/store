/**
 * Wyze Camera Feed Card
 *
 * feed-card canvas for wyze_cam entities.
 * Auto-plays WebRTC live stream directly in the feed card.
 *
 * Props follow the feed-card contract:
 *   { uri, entityType, pathSegments, title, subtitle, metadata, size }
 *
 * Config is read from plugin storage (shared with drawer setup flow).
 */
...
function StreamCard({ config, cameraName, cameraNickname }: { config: BridgeConfig; cameraName: string; cameraNickname: string }) {
      ...
      try {
        ...
        ws.onmessage = async (msg) => {
          ...
          try {
            ...
            logger.error('WebRTC feed card: message error', { err: String(err) });
          }
        };
...
export default function WyzeFeedCard({ pathSegments, title }: FeedCardProps) {
  const [bridgeConfig] = usePluginStorage<BridgeConfig | null>('bridgeConfig', null);

  const cameraName = pathSegments[0] || '';
  const cameraNickname = title || cameraName;
  ...
  return <StreamCard config={bridgeConfig} cameraName={cameraName} cameraNickname={cameraNickname} />;
}
