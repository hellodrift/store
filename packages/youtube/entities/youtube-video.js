...
var require_youtube_search_api = __commonJS({
  "node_modules/youtube-search-api/index.js"(exports2) {
    var axios = require_axios2();
    var youtubeEndpoint = `https://www.youtube.com`;
    var GetYoutubeInitData = async (url) => {
      var initdata = await {};
    ...
    var GetYoutubePlayerDetail = async (url) => {
      var initdata = await {};
      try {
        const page = await axios.get(encodeURI(url));
    ...
    var GetData = async (keyword, withPlaylist = false, limit = 0, options = []) => {
      let endpoint = await `${youtubeEndpoint}/results?search_query=${keyword}`;
      try {
        if (Array.isArray(options) && options.length > 0) {
          const type = options.find((z2) => z2.type);
        ...
        const sectionListRenderer = await page.initdata.contents.twoColumnSearchResultsRenderer.primaryContents.sectionListRenderer;
        let contToken = await {};
        let items = await [];
    ...
    var nextPage = async (nextPage2, withPlaylist = false, limit = 0) => {
      const endpoint = await `${youtubeEndpoint}/youtubei/v1/search?key=${nextPage2.nextPageToken}`;
      try {
        const page = await axios.post(
          encodeURI(endpoint),
          nextPage2.nextPageContext
        );
        ...
        const itemsResult = limit != 0 ? items.slice(0, limit) : items;
        return await Promise.resolve({ items: itemsResult, nextPage: nextPage2 });
      } catch (ex) {
        await console.error(ex);
        return await Promise.reject(ex);
      }
    };
    var GetPlaylistData = async (playlistId, limit = 0) => {
    ...
    var GetSuggestData = async (limit = 0) => {
    ...
    var GetChannelById = async (channelId) => {
    ...
    var GetVideoDetails = async (videoId) => {
      ...
      try {
        ...
        const res = await {
          ...
          isLive: firstContent.viewCount.videoViewCountRenderer.hasOwnProperty(
            "isLive"
          ) ? firstContent.viewCount.videoViewCountRenderer.isLive : false,
    ...
    var GetShortVideo = async () => {
      const page = await GetYoutubeInitData(youtubeEndpoint);
      const shortResult = await page.initdata.contents.twoColumnBrowseResultsRenderer.tabs[0].tabRenderer.content.richGridRenderer.contents.filter((x) => {
        return x.richSectionRenderer;
      }).map((z2) => z2.richSectionRenderer.content).filter((y) => y.richShelfRenderer).map((u) => u.richShelfRenderer).find((i) => i.title.runs[0].text == "Shorts");
      const res = await shortResult.contents.map((z2) => z2.richItemRenderer).map((y) => y.content.reelItemRenderer);
    ...
    exports2.GetListByKeyword = GetData;
    exports2.NextPage = nextPage;
    exports2.GetPlaylistData = GetPlaylistData;
    exports2.GetSuggestData = GetSuggestData;
    exports2.GetChannelById = GetChannelById;
    exports2.GetVideoDetails = GetVideoDetails;
    exports2.GetShortVideo = GetShortVideo;
  }
});

// entities/youtube-video.ts
var youtube_video_exports = {};
__export(youtube_video_exports, {
  default: () => youtube_video_default
});
module.exports = __toCommonJS(youtube_video_exports);
var import_zod = require("zod");
var import_entity_sdk = require("@drift/entity-sdk");
var youtubeVideoSchema = import_zod.z.object({
  id: import_zod.z.string(),
  type: import_zod.z.literal("youtube_video"),
  uri: import_zod.z.string(),
  title: import_zod.z.string(),
  channelTitle: import_zod.z.string(),
  thumbnailUrl: import_zod.z.string(),
  description: import_zod.z.string().optional(),
  publishedAt: import_zod.z.string().optional(),
  embedUrl: import_zod.z.string(),
  watchUrl: import_zod.z.string()
});
...
var openVideoInput = import_zod.z.object({
  videoId: import_zod.z.string().describe("The YouTube video ID to open")
});
var YoutubeVideoEntity = (0, import_entity_sdk.defineEntity)({
  type: "youtube_video",
  ...
  uriPath: {
    segments: ["videoId"],
    parse: (segments) => ({ videoId: segments[0] }),
  ...
  actions: [
    ...
    },
    {
      ...
      inputSchema: import_zod.z.object({
        query: import_zod.z.string().describe("Search query for YouTube videos"),
        limit: import_zod.z.number().int().min(1).max(10).optional().describe("Number of results (default 5)")
      }),
  ...
  search: async (query, options, ctx) => {
    const limit = options?.limit ?? 5;
    ctx.logger.info("Searching YouTube", { query, limit });
    try {
      const YoutubeSearchApi = require_youtube_search_api();
      ...
      for (const item of items) {
        ...
        const thumbUrl = thumbnails.length > 1 ? thumbnails[Math.min(1, thumbnails.length - 1)].url : buildThumbnail(videoId);
        videos.push({
          id: videoId,
          type: "youtube_video",
          uri: `@drift//youtube_video/${videoId}`,
...
var youtube_video_default = YoutubeVideoEntity;
